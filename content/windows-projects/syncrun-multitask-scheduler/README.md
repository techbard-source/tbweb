...
4. Save - the INI file (`%LOCALAPPDATA%\SyncRun\SyncRun.ini`) is updated automatically.
5. Use the tray menu to run tasks manually or pause the scheduler.

---

## Using SyncRun for Remote Backups
...
- **Settings change detection** - an asterisk `*` appears in the window title when unsaved changes exist; closing prompts a confirmation dialog.
- **Save** - writes the INI with UTF-8 BOM, preserving unknown sections and comments.

---

## Delay Prediction Screen
The Settings editor includes a delay prediction screen to help you identify schedule congestion before it affects execution.

- **Open** - In Settings, click **Simulator**.
- **Monthly calendar** - Each day shows how many task deferrals are expected.
- **Color indicators** - Low-risk days are shown in safer colors; high-risk days are highlighted.
- **Planning support** - Use this view to move task times and reduce overlapping runs.

This feature is especially useful when multiple HOURLY/DAILY tasks overlap within similar time windows.

---
## Common Commands
1. **Pause/Resume** - left-click the tray icon.
2. **Run task manually** - right-click menu -> task name.
3. **Open Settings** - right-click -> *Settings*.

---

## Troubleshooting
- **Task does not run** - check the log for `NextRun` calculation errors or missing `ExePath`.
- **Auto-start fails** - ensure `AutoStart=1` and the registry key `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` points to `SyncRun.exe`.
